MODULE dynnxt
   ...
   !! $Id: dynnxt.F90 10425 2018-12-19 21:54:16Z smasson $ 
...
CONTAINS
   ...
   SUBROUTINE dyn_nxt ( kt )
      ...
      REAL(wp) ::   rn_wd_bctaper, zwgtu, zwgtv
      REAL(wp), ALLOCATABLE, DIMENSION(:,:)   ::   zue, zve
      REAL(wp), ALLOCATABLE, DIMENSION(:,:,:) ::   ze3u_f, ze3v_f, zua, zva 
      REAL(wp), ALLOCATABLE, DIMENSION(:,:) :: ztwdmask, zuwdmask, zvwdmask
      !!----------------------------------------------------------------------
      !
      ...
      IF ( ln_dynspg_ts ) THEN
         ...
         ENDIF
                 ! > JC WAD
         IF ( ll_wd.AND.ln_wd_dl.AND.ln_wd_dl_bc ) THEN
            rn_wd_bctaper = 1._wp
            ALLOCATE( ztwdmask(jpi,jpj), zuwdmask(jpi,jpj), zvwdmask(jpi,jpj))
            CALL wad_tmsk( ssha(:,:), ztwdmask )
            CALL wad_Umsk( ztwdmask, ua_b, va_b, ua_b, va_b, zuwdmask, zvwdmask )
            DO jj = 1, jpj
               DO ji = 1, jpi
                  zwgtu = zuwdmask(ji,jj) * MAX(MIN(1._wp, 2._wp * hu_a(ji,jj)/rn_wd_bctaper - 1._wp), 0._wp)
                  zwgtv = zvwdmask(ji,jj) * MAX(MIN(1._wp, 2._wp * hv_a(ji,jj)/rn_wd_bctaper - 1._wp), 0._wp)
                  ua(ji,jj,:) = (zwgtu * ua(ji,jj,:) &
                     &        + (1._wp - zwgtu) * ua_b(ji,jj))*umask(ji,jj,:)
                  va(ji,jj,:) = (zwgtv * va(ji,jj,:) &
                     &        + (1._wp - zwgtv) * va_b(ji,jj))*vmask(ji,jj,:)
               END DO
            END DO
            DEALLOCATE( ztwdmask, zuwdmask, zvwdmask)
         ENDIF 
      ENDIF
