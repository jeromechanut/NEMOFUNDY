...
#  include "vectopt_loop_substitute.h90"
   ...
   !! $Id: diawri.F90 11536 2019-09-11 13:54:18Z smasson $
...
CONTAINS
   ...
   SUBROUTINE dia_wri( kt )
      ...
      IF( ll_wd ) THEN
         DO jj = 1, jpj
            DO ji = 1, jpi
               IF (ht_0(ji,jj)>ssh_ref) THEN  
                  z2d(ji,jj) = (sshn(ji,jj) +  ssh_ref ) * tmask(ji,jj,1)
               ELSE
                  z2d(ji,jj) = (sshn(ji,jj) + ht_0(ji,jj)) * tmask(ji,jj,1) 
               ENDIF
            END DO
         END DO       
         CALL iom_put( "ssh", z2d )
